...
import { AllGenreParameter } from "./condition";
...
function LandingPage() {
    ...
    const loadVideoLists = async () => {
      setLoading(true);
      try {
        const response = await axios.get(`${config.endpoint}v1/videos`);
    ...
    const handleSort = async parameter => {
      ...
      try {
        const response = await axios.get(
          `${config.endpoint}v1/videos?sortBy=${parameter}`
        );
        if (response.status === 200) {
    ...
    const genreFilterHandler = async item => {
      ...
      try {
        ...
        const response = await axios.get(
          `${config.endpoint}v1/videos?genres=${searchKeyWords}`
        );
  ...
  return (
    <>
        ...
        <FilterData handleSorting={handleSort} videoLists={videoList} genreFilterHandler={genreFilterHandler}/>
            <Container>
            {
                loading ?
